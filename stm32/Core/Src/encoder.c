...
volatile uint32_t ENCODER_POS_DIRECTIONAL_FORWARD[2] = {0}; // timer readout, compensated for opp rotations
volatile uint32_t ENCODER_POS_DIRECTIONAL_BACKWARD[2] = {0};
...
uint32_t _read_encoder_side_mm(uint8_t side) {
	...
	if(side == 1) {
		...
		ENCODER_POS_DIRECTIONAL_FORWARD[side] = *encoder_side;
	}
	else if(side == 0) {
		encoder_side = MOTOR_ENCODER_READOUT_LEFT;
		ENCODER_POS[side] = *encoder_side;
		ENCODER_POS_DIRECTIONAL_FORWARD[side] = MOTOR_ENCODER_MAX_POS - *encoder_side;
	}
	else return 0;
...
void encoder_reset_counters_backward() {
	...
	*MOTOR_ENCODER_READOUT_RIGHT = MOTOR_ENCODER_MAX_POS;
	...
	ENCODER_POS_DIRECTIONAL_FORWARD[0] = MOTOR_ENCODER_MAX_POS * MOTOR_ENCODER_CONSTANT;
	ENCODER_POS_DIRECTIONAL_FORWARD[1] = MOTOR_ENCODER_MAX_POS * MOTOR_ENCODER_CONSTANT;
	ENCODER_POS_DIRECTIONAL_BACKWARD[0] = 0;
	ENCODER_POS_DIRECTIONAL_BACKWARD[1] = 0;
}
...
void encoder_reset_counters_forward() {
	...
	*MOTOR_ENCODER_READOUT_RIGHT = 0;
	ENCODER_POS[0] = 0;
	ENCODER_POS[1] = 0;
	ENCODER_POS_DIRECTIONAL_FORWARD[0] = 0;
	ENCODER_POS_DIRECTIONAL_FORWARD[1] = 0;
	ENCODER_POS_DIRECTIONAL_BACKWARD[0] = MOTOR_ENCODER_MAX_POS * MOTOR_ENCODER_CONSTANT;
	ENCODER_POS_DIRECTIONAL_BACKWARD[1] = MOTOR_ENCODER_MAX_POS * MOTOR_ENCODER_CONSTANT;
}
...
void encoder_poll(void) {
	...
	ENCODER_POS_DIRECTIONAL_FORWARD[0] = (MOTOR_ENCODER_MAX_POS - *MOTOR_ENCODER_READOUT_LEFT) % (MOTOR_ENCODER_MAX_POS) * MOTOR_ENCODER_CONSTANT;
	ENCODER_POS_DIRECTIONAL_FORWARD[1] = *MOTOR_ENCODER_READOUT_RIGHT % (MOTOR_ENCODER_MAX_POS) * MOTOR_ENCODER_CONSTANT;

	ENCODER_POS_DIRECTIONAL_BACKWARD[0] = *MOTOR_ENCODER_READOUT_LEFT % MOTOR_ENCODER_MAX_POS * MOTOR_ENCODER_CONSTANT;
	ENCODER_POS_DIRECTIONAL_BACKWARD[1] = (MOTOR_ENCODER_MAX_POS - *MOTOR_ENCODER_READOUT_RIGHT) % (MOTOR_ENCODER_MAX_POS) * MOTOR_ENCODER_CONSTANT;
